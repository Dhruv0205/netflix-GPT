...
return (
    ...
    <form onSubmit={(e) => e.preventDefault()} className='absolute p-12 bg-black w-3/12 my-36 mx-auto right-0 left-0 text-white rounded-md bg-opacity-85'>
    <h1 className='font-bold text-3xl py-3'>{!isLogin ?" Sign up ": "Sign In"}</h1>
    {!isLogin && <input type="text" placeholder='Full Name' ref={name} className='p-4 my-3 w-full bg-gray-800 rounded-md'  /> }
    <input type="email" placeholder='Email Address' className='p-4 my-4 w-full bg-gray-800 rounded-md' ref={email}  />
    <input type="password" placeholder='Password' className='p-4 mb-8 w-full bg-gray-800 rounded-md' ref={password}/>
    <p className='text-lg text-red-500 font-bold py-2'>{errorMessage}</p>
    <button className='bg-red-700 p-4 w-full my-6 rounded-lg' onClick={buttonClickHandler}>{isLogin ?"Sign In" : "Sign up"}</button>
